...
Imported.Bbzs_SimpleCrops = true;

var Bbzs = Bbzs || {};                  // Bbzs's main object
Bbzs.CROPS = Bbzs.CROPS || {};          // Bbzs's stuff

//-----------------------------------------------------------------------------
/*:
 * @plugindesc (v.1.4) A simple farming/growing crop system.
 * 
 * @author Bbzs - Bbzss-scripts.com
 *
 ...
 * ----------------------------------------------------------------------------
 *
 *    Bbzs.CROPS.isCrop(eId)     // Check if event has <crop> note tag
 * 
 *    Bbzs.CROPS.hasSeed(eId)    // Check if event has a seed planted
 *
 *    Bbzs.CROPS.isReady(eId)    // Check if event seed is ready to harvest
 *
 *    Bbzs.CROPS.isSeed(iId)     // Check if item has <seed> tag and is a seed
 *
 * ----------------------------------------------------------------------------
 *  SCRIPT calls
 * ----------------------------------------------------------------------------
 *
 *    Bbzs.CROPS.harvest(eId,v);  // Harvest an event's crop.
 *                                // v refers to a variable id to store the
 *                                // NAME of the ITEM obtained from the crop
 *                                // as a string to use in messages with \v[x]
 *
 *    Bbzs.CROPS.remove(eId);     // Remove the plant from an event.
 *
 *    Bbzs.CROPS.plant(eId,iId,vId);  // plant a seed item into an event.
 ...
 *
 *    Bbzs.CROPS.water(eId,vId);   // water the crop event starting now
 ...
 *
 *    Bbzs.CROPS.waterAll(vId);  // same as above but applies to all events on
 *                               // the current map.
 *
 * ----------------------------------------------------------------------------
 ...
 * ----------------------------------------------------------------------------
 *
 *    Bbzs.CROPS.update();   // update all crops on the map
 *
 * ----------------------------------------------------------------------------  
 */
...
Bbzs.CROPS.img = PluginManager.parameters('Bbzs_SimpleCrops')["Crop Charset Name"];
Bbzs.CROPS.wateredTime = Number(PluginManager.parameters('Bbzs_SimpleCrops')["Watered Time"]);
Bbzs.CROPS.wateredBenefit = Number(PluginManager.parameters('Bbzs_SimpleCrops')["Watered Benefit"]);

Bbzs.CROPS.useFix = PluginManager.parameters('Bbzs_SimpleCrops')["Crop Under or Front Fix"] === 'true' ? true : false;


var txt = PluginManager.parameters('Bbzs_SimpleCrops')["Crop Priority"].split(',');
Bbzs.CROPS.prio = [1,1,1,1,1];
for (var i = 0; i < txt.length; i++) {
	Bbzs.CROPS.prio[i] = Number(txt[i]);
}

Bbzs.CROPS.isCrop = function(eventId) {
	return $gameMap.event(eventId) && $gameMap.event(eventId).crop();
};

Bbzs.CROPS.hasSeed = function(eventId) {
	return Bbzs.CROPS.isCrop(eventId) && $gameMap.event(eventId).crop()._seed;
};

Bbzs.CROPS.isReady = function(eventId) {
	return Bbzs.CROPS.hasSeed(eventId) ? $gameMap.event(eventId).crop().isReady() : false;
};

Bbzs.CROPS.harvest = function(eventId,varId) {
	$gameVariables._data[varId] = 0;
	if (Bbzs.CROPS.isReady(eventId)) {
...
Bbzs.CROPS.remove = function(eventId,keepWatered) {
	if (Bbzs.CROPS.isCrop(eventId)) {
		$gameMap.event(eventId).crop().resetPlot(!keepWatered); // remove crop but keep watered
		$gameMap.event(eventId).updateCropGraphic();
	}
};

Bbzs.CROPS.water = function(eventId,daysVar) {
	if (Bbzs.CROPS.isCrop(eventId)) {
...
Bbzs.CROPS.waterAll = function(daysVar) {
	var events = $gameMap.events();
	for (var i = 0; i < events.length; i++) {
		if (events[i]) Bbzs.CROPS.water(events[i]._eventId,daysVar);
	}
};

Bbzs.CROPS.plant = function(eventId,itemId,daysVar) {
	$gameParty.loseItem($dataItems[itemId],1);
	$gameMap.event(eventId).plantCrop(itemId,daysVar);
};

Bbzs.CROPS.isSeed = function(itemId) {
	return $dataItems[itemId] && $dataItems[itemId].meta.seed;
};

Bbzs.CROPS.update = function(mapId) {
	var events = $gameMap.events();
	for (var e in events) {
...
Bbzs.CROPS.Game_System_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
	Bbzs.CROPS.Game_System_initialize.call(this);
...
Game_Crop.prototype.waterPlot = function(daysVar) {
	...
	if (this._seed) this._growTime -= Bbzs.CROPS.wateredBenefit;
	this._dryTime = this._wateredTime + Bbzs.CROPS.wateredTime;
};
...
Bbzs.CROPS.Scene_Load_onLoadSuccess = Scene_Load.prototype.onLoadSuccess;
Scene_Load.prototype.onLoadSuccess = function() {
	Bbzs.CROPS.reSetupCropEvents = true; // set crops to reload graphics on load
	Bbzs.CROPS.Scene_Load_onLoadSuccess.call(this);
};
...
Bbzs.CROPS.Scene_Map_start = Scene_Map.prototype.start;
Scene_Map.prototype.start = function() {
    Bbzs.CROPS.Scene_Map_start.call(this);
	this.setupCropEvents();
};

Scene_Map.prototype.setupCropEvents = function() {
	if (Bbzs.CROPS.reSetupCropEvents) {
		var events = $gameMap.events();
		for (var e in events) {
			if (events[e].setupCropEvent) events[e].setupCropEvent($gameMap._mapId,e._eventId);
		}
	}
	Bbzs.CROPS.reSetupCropEvents = false;
};
...
Bbzs.CROPS.Game_Event_initialize = Game_Event.prototype.initialize;
Game_Event.prototype.initialize = function(mapId, eventId) {
	Bbzs.CROPS.Game_Event_initialize.call(this,mapId,eventId);
...
Game_Event.prototype.updateCropGraphic = function() {
	...
	if (crop._seed) {
		...
		// No seed planted
		this._priorityType = Bbzs.CROPS.prio[0];
		var charset = 0;
		var pat = 1;
		var ind = 0;
		var dir = crop.isWatered() ? 4 : 2;
	...
	this._direction = dir;
	this._pattern = pat;
	this._characterName = Bbzs.CROPS.img + charset;
...
Bbzs.CROPS.Game_Map_setup = Game_Map.prototype.setup;
Game_Map.prototype.setup = function(mapId) {
	$gameSystem._crops[mapId] = $gameSystem._crops[mapId] || {};
	Bbzs.CROPS.Game_Map_setup.call(this,mapId);
};



if (Bbzs.CROPS.useFix) {

Bbzs.CROPS.Game_Player_checkEventTriggerHere = Game_Player.prototype.checkEventTriggerHere;
Game_Player.prototype.checkEventTriggerHere = function(triggers) {
	if (Input.isTriggered('ok')) this.checkCropEventThere(triggers);
	if ($gameMap.setupStartingEvent()) return true;
	Bbzs.CROPS.Game_Player_checkEventTriggerHere.call(this,triggers);
};
